...
5. If a practice exam is attached, describe its structure in `exam_blueprint` (sections by type and points, in exam order, plus `duration_minutes` if the exam states a time limit) and make the pool at least three times each section's count, with exactly the section's points, spread over every topic, so PrepCanvas can compose many different exam variants.
